angular.module('beAppli.services', [])
...
.service('PostService', function ($rootScope, $http, $q, WORDPRESS_API_URL, AuthService, BookMarkService){
  ...
  this.getPostsFromAuthor = function(authorId, page) {
    var deferred = $q.defer();

		$http.jsonp(WORDPRESS_API_URL + 'get_author_posts/' +
          '?id='+ authorId +
     '&page='+ page +
         '&callback=JSON_CALLBACK')
  ...
  this.getPostsFromTag = function(tagId, page) {
    var deferred = $q.defer();

		$http.jsonp(WORDPRESS_API_URL + 'get_tag_posts/' +
          '?id='+ tagId +
     '&page='+ page +
         '&callback=JSON_CALLBACK')
